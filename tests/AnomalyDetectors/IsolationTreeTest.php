...
class IsolationTreeTest extends TestCase
{
    ...
    public function test_make_prediction()
    {
        ...
        $outcomes = $this->testing->possibleOutcomes();

        $this->assertContains($results[0], $outcomes);
        $this->assertContains($results[1], $outcomes);
        $this->assertContains($results[2], $outcomes);
        $this->assertContains($results[3], $outcomes);
    }
    ...
    public function test_predict_proba()
    {
        ...
        $this->assertThat($results[0], $this->logicalAnd(
            $this->greaterThanOrEqual(0), $this->lessThanOrEqual(1))
        );

        $this->assertThat($results[1], $this->logicalAnd(
            $this->greaterThanOrEqual(0), $this->lessThanOrEqual(1))
        );

        $this->assertThat($results[2], $this->logicalAnd(
            $this->greaterThanOrEqual(0), $this->lessThanOrEqual(1))
        );

        $this->assertThat($results[3], $this->logicalAnd(
            $this->greaterThanOrEqual(0), $this->lessThanOrEqual(1))
        );
    }
}
